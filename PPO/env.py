...
class tradingEng(gym.Env):
    def __init__(self, paths):
        ...
        lower = np.concatenate([-1*np.ones(2), 15*np.ones(1), np.zeros(1), -15*np.ones(1)])
        upper = np.concatenate([np.ones(2), 15*np.ones(1), np.ones(1), 15*np.ones(1)]) #pos, swap value, Q value, r
        ...
        lowera = np.concatenate([-1*np.ones(2)])
        uppera = np.concatenate([1*np.ones(2)])
    ...
    def reset(self, seed = None, options = None):
        ...
        init_ratio  = np.ones(2)*(1/2)    # the initial value distribution, should probably be a delta hedge
        action = self.vec_to_dict_act(init_ratio)
    ...
    def prices(self):
        swa = [self.currpth.Swaptions[8][self.tIDX]]
        qs = [self.currpth.Q_s[8][self.tIDX]]
    ...
    def vec_to_dict_act(self, action):
        if not len(action) == 2:
    ...
    def step(self, action):
        ...
        if not isinstance(actionl, dict):
            actionl = self.vec_to_dict_act(actionl)
